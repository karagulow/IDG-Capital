...
function renderList(items) {
  ...
  if (!items.length) {
    listEl.innerHTML = '<li class="caption">결과 없음</li>';
  ...
  items.slice(0, visibleCount).forEach(({ name, logo, sector, region }) => {
    ...
    li.innerHTML = `
			...
			<div class="businesses__item--tags">
				<p class="caption businesses__item--tag">${regionLabels[region] || region}</p>
				<p class="caption businesses__item--tag">${sectorLabels[sector] || sector}</p>
			</div>
		`;
...
filterGroups.forEach((group) => {
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (filterType === "sector") filters.sector = value;
      if (filterType === "region") filters.region = value;
